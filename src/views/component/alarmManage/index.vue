 ...
 * @LastEditTime: 2023-07-25 14:04:07
 * @FilePath: \safe-product-alarm-browser\src\views\component\alarmManage\index.vue
 * @Description: 这是默认设置,请设置`customMade`, 打开koroFileHeader查看配置 进行设置: https://github.com/OBKoro1/koro1FileHeader/wiki/%E9%85%8D%E7%BD%AE
-->
<template>
  <div class="ya-box">
    <YaEditForm :formColumns="formColumns" @submit="handlerToSubmit" :loading="loading"></YaEditForm>
  </div>
</template>
  ...
  queryType().then(res => {
    ...
    if (code == 200 && data) {
      ...
      params.forEach(item => {
        ...
        item.status == 1 && formColumns.value[i].value.push(item.typeCode)
